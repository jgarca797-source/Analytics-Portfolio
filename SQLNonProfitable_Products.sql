...
resumo_geral AS (
    SELECT
        Ano,
        Mes,
        SUM(CustoMensalBruto) AS CustoMensalBruto,
        SUM(ReceitaMensalBruta) AS ReceitaMensalBruta,
        SUM(LucroBrutoMensal) AS LucroBrutoMensal,
        SUM(LucroOperacional) AS LucroOperacional
...
SELECT 
    v.ProductID,
    p.Name AS Produto,
    SUM(v.ReceitaMensalBruta) AS ReceitaTotal,
    SUM(v.CustoMensalBruto) AS CustoTotal,
    SUM(v.LucroBrutoMensal) AS LucroBrutoTotal,
    CASE 
        WHEN SUM(v.ReceitaMensalBruta) = 0 THEN 0
        ELSE ROUND(SUM(v.LucroBrutoMensal) * 100.0 / SUM(v.ReceitaMensalBruta), 2)
    END AS MargemBrutaPercentual
FROM vendas_agg v
JOIN regioes r ON v.TerritoryID = r.TerritoryID
LEFT JOIN Production.Product p ON v.ProductID = p.ProductID
GROUP BY v.ProductID, p.Name
HAVING SUM(v.LucroBrutoMensal) < 0
